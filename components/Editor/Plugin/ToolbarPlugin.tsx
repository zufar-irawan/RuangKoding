...
export function ToolbarPlugin() {
  ...
  const updateToolbar = useCallback(() => {
    ...
    if ($isRangeSelection(selection)) {
      ...
      if (
        typeof (element as { getFormatType?: () => string }).getFormatType ===
        "function"
      ) {
        const format = (
          element as { getFormatType: () => string }
        ).getFormatType() as
          | "left"
          | "center"
          | "right"
  ...
  const formatBlock = (formatType: keyof typeof blockTypeToBlockName) => {
      ...
      if ($isRangeSelection(selection)) {
        if (formatType === "paragraph") {
          ...
          $setBlocksType(selection, () =>
            $createHeadingNode(formatType as HeadingTagType),
          );
  ...
  const createFormatHandler = useCallback(
